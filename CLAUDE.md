...
Standalone benchmarking/analysis entry points (not part of the data-prep pipeline, run manually against the API):

```bash
uv run python scripts/service/benchmark.py       # end-to-end estimation benchmark against a running API
uv run python scripts/service/ml_benchmark.py    # ML-only benchmark
uv run python scripts/service/latency.py         # latency measurement
```
...
**Logging**: `src/config/logging.py::configure_logging()` sets up timestamped console logging (`force=True`, since uvicorn/joblib workers may already have attached handlers) and is called at every real entrypoint - FastAPI startup, `scripts/prepare.py`, every `datasets/<name>/prepare.py`/`transform.py`, and the `scripts/service/*.py` benchmarking tools - since each runs as its own OS process/subprocess and doesn't inherit another process's logging config. Verbosity defaults to `LOG_LEVEL` (`INFO` if unset); set `LOG_LEVEL=DEBUG` to see per-file ZIP extraction and subprocess command lines from `scripts/_common.py`.

**`src/benchmarking.py`** holds shared metric/fold helpers (`regression_metrics`, `group_k_fold_indices`, `group_k_fold_test_groups`) used by both the notebooks in `reports/` and the standalone scripts in `scripts/service/`; grouping is always by `Crit_gen` to prevent leakage between folds.

**`reports/`** contains numbered Jupyter notebooks (`NN_description.ipynb`) that consume the `report-*.joblib` benchmark artifacts at the repo root and the `reports/*_report.csv` / `risk_coverage_*.csv` outputs. These are analysis/writeup, not part of the service runtime.
