...
def processor(index: int, in_dir: Path, ignore_rows: list[int]) -> tuple[pd.DataFrame, ...]:
    ...
    def prepare_tsa_dataset() -> pd.DataFrame:
        ...
        print(f"[{index=}] TSA drop {prev - len(tsa_long)}/{prev} entries ({(prev - len(tsa_long)) / prev * 100:.1f}%)")
    ...
    # ### Process FSA
    # fsa = pd.read_csv(in_dir / f"FSA_main_{index}.csv", sep=";", decimal=",", index_col=0)
...
def main(in_dir: Path, ignore_list_path: Path, dictionary_path: Path, out_dir: Path):
    ...
    for _lf, _tsa in tqdm(executor(jobs)):
        lf.append(_lf)
        tsa.append(_tsa)
