...
**Important follow-up finding (2026-07-25, see "Topology Matching Accuracy Ablation" below): the
`lines-only` filter did not improve typical conditional accuracy relative to no filter, and imposed
a large coverage cost; the observed maxima were lower in three matched samples, a limited
exploratory finding rather than a demonstrated general tail bound.** Read "the deployed choice"
language throughout this section as a physical-compatibility/coverage justification, not a claim
that it was shown to improve `CCT` estimates - it wasn't, on the data checked so far.
...
This paper's topology key targets network connectivity (which lines/buses are in service), treating
generator dispatch (which unit happens to be committed this hour) as a separate concept - conflating
them is why "full" fragments so badly (165 independently-switching generators multiply
combinatorially). Empirically checked on 2026-07-25: for all 165 generators
...
2. **Three named, selectable topology variants**, generated by `transform.py` as separate
   ...
   - `lines-only` - every `oserv_Lne*`/`oserv_Line*` column, generators excluded - **the deployed
     choice** going forward, and the one used for every ELES number reported in the paper
